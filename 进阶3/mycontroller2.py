...
tabel_name = p4info_helper.get_tables_name(entry.table_id)
print('%s: ' % tabel_name, end=' ')
for m in entry.match:
print(p4info_helper.get_match_field_name(tabel_name, m.field_id), en
...
"""
for response in sw.ReadCounters(p4info_helper.get_counters_id(counter_name),
for entity in response.entities:
...
#增加s1-s3,s2-s3的隧道转发规则
# Write the rules that tunnel traffic from h1 to h3
writeTunnelRules(p4info_helper, ingress_sw=s1, egress_sw=s3, tunnel_id=3
dst_eth_addr="08:00:00:00:03:33", dst_ip_addr="10.0.3.3
# Write the rules that tunnel traffic from h3 to h1
writeTunnelRules(p4info_helper, ingress_sw=s3, egress_sw=s1, tunnel_id=4
dst_eth_addr="08:00:00:00:01:11", dst_ip_addr="10.0.1.1"
# Write the rules that tunnel traffic from h2 to h3
writeTunnelRules(p4info_helper, ingress_sw=s2, egress_sw=s3, tunnel_id=5
dst_eth_addr="08:00:00:00:03:33", dst_ip_addr="10.0.3.3
# Write the rules that tunnel traffic from h3 to h2
writeTunnelRules(p4info_helper, ingress_sw=s3, egress_sw=s2, tunnel_id=6
dst_eth_addr="08:00:00:00:02:22", dst_ip_addr="10.0.2.2
# TODO Uncomment the following two lines to read table entries from s1 a
...
printCounter(p4info_helper, s1, "MyIngress.ingressTunnelCounter", 10
printCounter(p4info_helper, s2, "MyIngress.egressTunnelCounter", 100
 printCounter(p4info_helper, s2, "MyIngress.ingressTunnelCounter", 20
printCounter(p4info_helper, s1, "MyIngress.egressTunnelCounter", 200
printCounter(p4info_helper, s1, "MyIngress.ingressTunnelCounter", 30
printCounter(p4info_helper, s3, "MyIngress.egressTunnelCounter", 300
printCounter(p4info_helper, s3, "MyIngress.ingressTunnelCounter", 40
printCounter(p4info_helper, s1, "MyIngress.egressTunnelCounter", 400
printCounter(p4info_helper, s2, "MyIngress.ingressTunnelCounter", 50
printCounter(p4info_helper, s3, "MyIngress.egressTunnelCounter", 500
printCounter(p4info_helper, s3, "MyIngress.ingressTunnelCounter", 60
